...
const testGet = async (req, res) => {
  try {
    const result = await db.query(
      "SELECT * FROM note.notes LEFT JOIN note.notespermissions ON notes.id = notespermissions.note_id"
    );
    console.log(result);
...
const createNote = async (req, res) => {
  ...
  if (isNumber(ownerId)) {
    try {
      db.query(
        "INSERT INTO note.notes (note_header, note_body, owner_id) VALUES ($1, $2, $3)",
...
const getNotesByUserId = async (req, res) => {
  ...
  if (isNumber(userId)) {
    try {
      const result = await db.query(
        "SELECT * FROM note.notes LEFT JOIN note.notespermissions ON notes.id = notespermissions.note_id WHERE owner_id = $1 OR user_id = $1",
        [userId]
      );
      res.json(result.rows);
    } catch (err) {
...
const editNoteByNoteId = async (req, res) => {
  ...
  if (isNumber(noteId)) {
    try {
      db.query(
        "UPDATE note.notes SET note_header = $1, note_body = $2, date_last_edit = $3 WHERE id = $4",
...
const deleteNoteByNoteId = async (req, res) => {
  ...
  if (isNumber(noteId)) {
    try {
      db.query("DELETE FROM note.notes WHERE id = $1", [noteId]);
      res.status(200).send("OK");
    } catch (err) {
      console.log(err);
      res.status(500).send("error");
...
const checkUserExistInDB = async (userId) => {
  const result = await db.query("SELECT id FROM note.users WHERE id = $1", [
    userId,
  ]);
...
const createPermission = async (req, res) => {
  ...
  if (isNumber(noteId) && isNumber(userId) && isBoolean(canEdit)) {
    const isUserExists = await checkUserExistInDB(userId);
    if (isUserExists) {
      try {
        db.query(
          "INSERT INTO note.notespermissions (note_id, user_id, canedit) VALUES ($1, $2, $3)",
      ...
      res.status(400).send("User with id " + userId + " not exists");
    }
  } else {
    console.log(
      "'createPermission' - noteId is " +
        noteId +
        ", userId is " +
        noteId +
        ", canEdit is " +
...
const getPermissionByNoteId = async (req, res) => {
  ...
  if (isNumber(noteId)) {
    try {
      const result = await db.query(
        "SELECT * FROM note.notespermissions WHERE note_id = $1",
        [noteId]
      );
      res.json(result.rows);
    } catch (err) {
...
const editPermissionByPermissionId = async (req, res) => {
  ...
  if (isNumber(permissionId) && isNumber(userId) && isBoolean(canEdit)) {
    try {
      db.query(
        "UPDATE note.notespermissions SET user_id = $1, canedit = $2 WHERE id = $3",
    ...
    console.log(
      "'editPermissionByPermissionId' - permissionId is " +
        permissionId +
        ", userId is " +
        userId +
        ", canEdit is " +
...
const deletePermissionByPermissionId = async (req, res) => {
  ...
  if (isNumber(permissionId)) {
    ...
    console.log(
      "'deletePermissionByPermissionId' - permissionId is " + permissionId
    );
...
const login = async (req, res) => {
  ...
  try {
    const result = await db.query(
      "SELECT id, password, isactive FROM note.users WHERE login = $1",
      [login]
    );
    if (result.rowCount == 1) {
...
const createUser = async (req, res) => {
  ...
  try {
    db.query(
      "INSERT INTO note.users (login, name, surname, password) VALUES ($1, $2, $3, $4)",
...
const getUserByUserId = async (req, res) => {
  ...
  if (isNumber(userId)) {
    try {
      const result = await db.query(
        "SELECT name, surname, isadmin, isactive FROM note.users WHERE id = $1",
        [userId]
      );
      res.json(result.rows);
    } catch (err) {
...
const editUserPasswordByUserId = async (req, res) => {
  ...
  if (isNumber(userId)) {
    try {
      db.query("UPDATE note.users SET password = $1 WHERE id = $2", [
        newPassword,
        userId,
      ]);
      res.status(200).send("OK");
...
const disableUserByUserId = async (req, res) => {
  ...
  if (isNumber(userId)) {
    try {
      db.query("UPDATE note.users SET isactive = false WHERE id = $1", [
        userId,
      ]);
      res.status(200).send("Ok");
    } catch (err) {
    ...
    const result = await db.query(
      "SELECT id FROM note.notes WHERE owner_id = $1",
      [userId]
    );
    const values = result.rows.map((row) => row.id);
    values.forEach((element) =>
      db.query("DELETE from note.notespermissions WHERE note_id = $1", [
        element,
      ])
    );
  } else {
